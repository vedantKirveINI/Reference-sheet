...
- **Share Modal**: Google Sheets-inspired sharing dialog (`src/views/sharing/share-modal.tsx`) with clean section layout: (1) **Add People** — search input with multi-select pill/chip tags inline (avatar + name + X), shared role selector dropdown (Viewer/Editor with icons) to the right of search bar, invite button appears when users selected; search results show avatar + name + email with already-selected dimming; (2) **People with Access** — collapsible section with count badge, stacked avatar preview when collapsed, member list with role dropdowns (rounded-full style), dirty-state tracking (amber highlight + ring on modified rows); (3) **General Access** — section label + rounded card with large circle icon (emerald globe when enabled, muted lock when restricted), description text, dropdown button opening upward with icon options matching Google Sheets reference; (4) **Footer** — "Copy link" button (left, with checkmark feedback) + "Done" button (right) or Cancel/Save when changes detected. Hooks: `useShareModal` (member/general-access state, save via `/asset/share`, cancel resets), `useSearchInvite` (300ms debounced search via `/user-sdk/search`, shared `inviteRole` for all selected users). API functions: `getShareMembers`, `inviteShareMembers`, `shareAsset`, `findOneAsset`, `searchUsers`.
