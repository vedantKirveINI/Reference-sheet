...
- **Share Modal**: Modern sharing dialog (`src/views/sharing/share-modal.tsx`) with three island-design sections: (1) **Invite People** — debounced search (300ms via lodash), pill/tag selection with per-user role dropdown, invite button calling `/asset/invite_members`; (2) **Existing Members** — color-coded avatars (8-color hash), owner crown badges, role dropdowns with dirty-state tracking (amber highlight on modified rows), sticky save/cancel footer appears when `hasChanges` is true; (3) **General Access** — radio toggle for "Restricted" vs "Anyone with the link", copy-link button with feedback. Hooks: `useShareModal` (member/general-access state, save via `/asset/share`, cancel resets), `useSearchInvite` (debounced search via `/user-sdk/search`, selected user management). API functions: `getShareMembers`, `inviteShareMembers`, `shareAsset`, `findOneAsset`, `searchUsers`. Props: `baseId` and `tableId` passed from App.tsx via `getIds()`. Modal opened via `useModalControlStore.openShareModal()` from header Share button.
