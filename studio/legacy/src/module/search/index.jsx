...
const AddNodeComponent = ({
  ...
  const filterNodes = useCallback(
    (searchTextValue) => {
      ...
      const searchData = (arr, filterText, intentTypes = []) => {
        const fuseOptions = {
          keys: [
        ...
        if (intentTypes.length > 0) {
          const intentMatches = arr.filter((node) =>
            intentTypes.some((t) => 
      ...
      if (filterText || intentTypes.length > 0) {
        ...
        tabData?.forEach((tab) => {
          const result = searchData(tab.components, filterText, intentTypes);
          if (result.length > 0) {
            resultCount += result.length;
            filteredComponents.push({
              ...tab,
              components: result.map(({ item }) => item),
            });
          }
        });
          ...
          <div className={styles.searchContainer}>
            <div className={styles.searchWrapper}>
              ...
              {searchText ? (
                ...
                >
                  <Icon outeIconName="OUTECloseIcon" sx={{ fontSize: "0.875rem" }} />
                </button>
              ) : (
