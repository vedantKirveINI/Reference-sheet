...
module.exports = async (flow, task_id, state, options) => {
  try {
    ...
    let body = await flow_utility_instance.parseIOV2(state, task_obj?.inputs);
    let req_options = {
      max_retry: 3,
      retry_delay_ms: 500,
      use_backoff_retry: true,
      method: "POST",
      url: `${api_info?.url}${api_info?.composer_path}`,
      headers: {
    ...
    if (![200, 201].includes(req_resp?.status_code)) {
      throw req_resp?.result?.result || req_resp?.result;
    }
    if(req_resp?.result?.status !== "success"){
      throw req_resp?.result?.result || {message: "Something went wrong"};
    }
