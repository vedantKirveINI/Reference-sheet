...
module.exports = (flow, task_id, state, options) => {
  return new Promise(async (resolve, reject)=> {
    ...
    try {
      ...
      let limit = flow_utility_instance.parseAndValidate({value: task_config?.limit, type: "INT"})?.value;
      let offset = flow_utility_instance.parseAndValidate({value: task_config?.offset, type: "INT"})?.value;
      let order_by = _.filter(task_config?.orderBy, input => { return (input?.checked === true); });
      let group_by = _.filter(task_config?.groupBy, input => { return (input?.checked === true); });
      ...
      //generate group by
      const parsed_group_by = flow_utility_instance.generateGroupByClause(state, group_by, client_info?.orm_type, client_info?.db_type);
      find_options.group = parsed_group_by.group_by;
      if(!(find_options.group?.length)) {
        delete find_options.group;
      }
      //Set limit and offset
      if(utility_instance.isEmptyOrNull(limit) === false) {
        find_options.limit = limit;
      }
      if(utility_instance.isEmptyOrNull(offset) === false) {
      ...
      const node_input = {
        ...
        group_by: parsed_group_by.group_by_str,
      ...
      result = await db_handler_instance.crud.find(client_info?.orm_type, model, find_options);
