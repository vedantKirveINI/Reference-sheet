...
import {
  getZipCountryDetailsContainerStyles,
  getZipCountryFlagImageStyles,
  getZipCountryCodeDisplayStyles,
} from "./zip-code-details-styles";
...
export interface ZipCodeInputInnerProps {
  theme?: any;
...
/**
 * Zip-code input inner: country selector (shadcn DropdownMenu) + zip input (InputMask).
 * Same data-testid as currency for useEnterKeyPress (country-input-autocomplete, etc.).
 */
export const ZipCodeInputInner = ({
  theme,
  disableInput,
  ...
  useEffect(() => {
    if (!value) {
      onChange({ zipCode: null, countryCode: "US" });
    }
  ...
  const handleInputChange = (e: any) => {
    ...
    onChange({ ...value, zipCode: zipCode || null }, e);
  };
  ...
  const handleCountrySelect = (option: any) => {
    ...
    onChange({ ...value, countryCode: option.countryCode, zipCode: null });
    setOpen(false);
    onDropdownOpenChange?.(false);
  };

  const zipPattern =
    ZIP_CODE_PATTERNS[value?.countryCode] || DEFAULT_ZIP_CODE_PATTERN;
  const mask = isCreator || !zipPattern?.pattern ? null : zipPattern.pattern;
  const formatChars = isCreator || !zipPattern?.formatChars ? null : zipPattern.formatChars;
  ...
  return (
    ...
    >
      ...
      >
        <DropdownMenuTrigger asChild>
          <button
            ...
            data-testid="country-details-for-ZIPCODE"
          >
            <img
              className={getZipCountryFlagImageStyles(viewPort)}
              src={getCountryFlagUrl(countries[value?.countryCode]?.countryCode)}
              alt={countries[value?.countryCode]?.countryCode}
              data-testid="country-flag-for-ZIPCODE"
            />
            <span
              className={getZipCountryCodeDisplayStyles(theme, viewPort).className}
              style={getZipCountryCodeDisplayStyles(theme, viewPort).style}
            >
              {countries[value?.countryCode]?.countryCode}
            </span>
            {!triggerDisabled && (
              <ChevronDown
      ...
      <div
        ...
        data-testid="zip-code-input-container"
      >
        <InputMask
          autoFocus={autoFocus}
          placeholder={placeholder || `e.g. ${zipPattern?.pattern ?? "99999"}`}
          mask={mask}
          formatChars={formatChars}
          value={isCreator ? (placeholder ?? "") : (value?.zipCode ?? "")}
