...
/**
 * Currency-only details styles (flag + currency code/symbol display).
 * Used only by the currency module. Responsive: smaller padding/gap on mobile.
 */
export const getCurrencyDetailsContainerStyles = ({
  disabled,
  theme,
  viewPort,
}: {
...
export const getCurrencyFlagImageStyles = (viewPort?: string): string => {
  const isMobile = viewPort === "MOBILE";
  const isCreatorDesktop = viewPort === "CREATOR_DESKTOP";
  return cn(
    "flex shrink-0 items-center justify-center gap-[0.625em] object-cover",
...
type DisplayStylesResult = { className: string; style?: { color?: string; fontFamily?: string } };

export const getCurrencyDisplayStyles = (theme: any, viewPort?: string): DisplayStylesResult => {
  const isMobile = viewPort === "MOBILE";
  const isCreatorDesktop = viewPort === "CREATOR_DESKTOP";
  return {
    className: cn(
      "flex min-w-0 items-center justify-center text-center font-normal tracking-[0.25px] leading-[140%]",
      isCreatorDesktop && "gap-[0.25em] text-[0.875em]",
      isMobile && !isCreatorDesktop && "gap-[0.35em] text-[1em]",
      !isMobile && !isCreatorDesktop && "gap-[0.5em] text-[1.25em]"
    ),
    style: {
