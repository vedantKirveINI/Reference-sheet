...
import { countries, getCountryFlagUrl } from "@oute/oute-ds.core.constants";
...
export interface CurrencyInputInnerProps {
  theme?: any;
  ...
  viewPort?: string; // kept for API compat; used by parent for mobile/desktop layout if needed
  value?: any;
  onFocus?: (e: any) => void;
  countryList?: any;
  onDropdownOpenChange?: (open: boolean) => void;
  /** When true (e.g. mobile preview), dropdown is a full-viewport overlay with back + search header */
...
/**
 * Currency input inner: country selector (shadcn DropdownMenu) + amount (shadcn Input).
 * All currency behaviour lives in the currency module; no country-input dependency.
 */
export const CurrencyInputInner = ({
  theme,
  disableInput,
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      let w = rect.width;
      // Cap so popover never exceeds parent container (width and right edge)
      if (parent) {
      ...
      // Also cap at space from trigger left to viewport right
      if (typeof window !== "undefined") {
        w = Math.min(w, Math.max(0, window.innerWidth - rect.left));
      }
      // Hard cap on mobile so dropdown never overflows the logical mobile frame
      if (viewPort === "MOBILE") {
    ...
    // Measure after layout so parent insets (mobile open) are applied
    let raf2: number;
  ...
  const filteredOptions = useMemo(() => {
    ...
    return options.filter((option: any) => {
      ...
      const currencyCode = option?.currencyCode?.toLowerCase();
      const countryNumber = option?.countryNumber?.toLowerCase();
      const currencySymbol = option?.currencySymbol ?? "";
      const symbolMatch =
        currencySymbol.toLowerCase().includes(term) ||
        currencySymbol.includes(termRaw);
      return (
  ...
  const handleInputChange = (e: any) => {
    ...
    let currencyValue = e.target.value < 0 ? 0 : e.target.value;
    const currentInput = e.nativeEvent?.data;

    if (currencyValue.length > 15) {
      return;
    }
  ...
  const handleCountrySelect = (option: any) => {
    // Ignore accidental selection from the same click that opened the dropdown
    if (isWithinGracePeriod()) return;
    // Ignore accidental selection when click-outside just closed the dropdown
  ...
  return (
    ...
    >
      ...
      >
        <DropdownMenuTrigger asChild>
          ...
          >
            ...
            >
              <span data-testid="currency-code">
                {countries[value?.countryCode]?.currencyCode}
              </span>
              <span data-testid="currency-symbol">
                {countries[value?.countryCode]?.currencySymbol}
              </span>
            </span>
            {!triggerDisabled && (
              <ChevronDown
                  ...
                  filteredOptions.map((option: any) => (
                    ...
                    >
                      ...
                      <img
                        ...
                        alt=""
                      />
                      <span className="text-[0.875em]">({option?.currencyCode})</span>
      ...
      >
        <Input
          value={isCreator ? placeholder : value?.currencyValue ?? ""}
          onChange={handleInputChange}
          type={isCreator ? "text" : "number"}
          placeholder={placeholder || "E.g. 10"}
          autoFocus={autoFocus}
          onFocus={(e) => onFocus?.(e)}
          disabled={disableInput}
          className={cn(
            "h-full w-full rounded-none border-0 border-b bg-transparent px-3 py-2 text-[1.25em] shadow-none outline-none focus-visible:ring-0 focus-visible:shadow-[inset_0_-2px_0_0_var(--border-color)] disabled:cursor-not-allowed disabled:opacity-50 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none",
