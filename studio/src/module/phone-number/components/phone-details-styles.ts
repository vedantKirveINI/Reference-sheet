...
/**
 * Phone trigger (flag + country number e.g. +91) styles. Tailwind + theme border color in style.
 */
export const getPhoneCountryDetailsContainerStyles = ({
  disabled,
  theme,
  viewPort,
}: {
...
export const getPhoneCountryFlagImageStyles = (viewPort?: string): string => {
  const isMobile = viewPort === "MOBILE";
  const isCreatorDesktop = viewPort === "CREATOR_DESKTOP";
  return cn(
...
type NumberDisplayResult = { className: string; style?: { color?: string; fontFamily?: string } };

export const getPhoneCountryNumberDisplayStyles = (theme: any, viewPort?: string): NumberDisplayResult => {
  const isMobile = viewPort === "MOBILE";
  const isCreatorDesktop = viewPort === "CREATOR_DESKTOP";
  return {
    className: cn(
