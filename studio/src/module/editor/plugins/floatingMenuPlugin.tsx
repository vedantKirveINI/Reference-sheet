...
function FloatingMenu({ id, show, ...props }: FloatingMenuProps) {
  ...
  useEffect(() => {
    ...
    const domRange = nativeSel.getRangeAt(0).getBoundingClientRect();
    setPos({ x: domRange.x, y: 100 });
    // computePosition(domRange, ref.current, { placement: "top" })
    //   .then((pos) => {
    //     console.log(domRange, ref.current, pos, nativeSel, "clndlskncsc");
  ...
  return (
    <div
      ...
      className={`lexical-floating-menu-container ${pos?.x && pos.y ? "lexical-visible" : "lexical-invisible"}`}
    >
      <IconButton
        icon="bold"
...
export function FloatingMenuPlugin({ id }) {
