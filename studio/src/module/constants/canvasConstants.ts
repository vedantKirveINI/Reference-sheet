...
let _canvasModeOverride: string | null = null;

export const setCanvasModeOverride = (mode: string | null) => {
  _canvasModeOverride = mode;
};

export const CANVAS_MODE = () => {
  if (_canvasModeOverride) return _canvasModeOverride;
  switch (getSubdomain()) {
    case "creator":
    ...
    default:
      return CANVAS_MODES.WC_CANVAS; // fallback
  }
};
