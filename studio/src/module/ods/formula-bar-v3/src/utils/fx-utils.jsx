...
import { FUNCTIONS } from "../constants/types.js";
...
const TYPE_COMPATIBILITY = {
  any: ['any', 'string', 'number', 'integer', 'boolean', 'array', 'object'],
  string: ['string', 'any'],
...
export const isTypeCompatible = (variableType, expectedType, options = {}) => {
  ...
  const normalizedVarType = variableType.toLowerCase();
  const normalizedExpectedType = expectedType.toLowerCase();
  
  if (normalizedVarType === normalizedExpectedType) {
    return true;
  }
  
  const compatibleTypes = TYPE_COMPATIBILITY[normalizedExpectedType] || [normalizedExpectedType, 'any'];
  if (compatibleTypes.includes(normalizedVarType)) {
    return true;
  }
  
  if (options.allowConversion && normalizedExpectedType === 'number' && normalizedVarType === 'string') {
    return true;
  }
...
export const filterVariablesByType = (variables, expectedType, functionContext = null) => {
  if (!variables || !expectedType || expectedType === 'any') {
    return variables;
  }
  ...
  if (functionContext && functionContext.currentFunction) {
    const { currentFunction, currentArgIndex, functionRegistry } = functionContext;
    const funcDef = functionRegistry?.[currentFunction];
    
    if (funcDef && funcDef.args && funcDef.args[currentArgIndex]) {
      const argDef = funcDef.args[currentArgIndex];
      effectiveExpectedType = argDef.type || 'any';
      
      if (funcDef.returnType === 'number' || funcDef.returnType === 'integer') {
  ...
  if (Array.isArray(variables)) {
    return variables.filter((variable) => {
      const varType = variable.returnType || variable.type || 'any';
      return isTypeCompatible(varType, effectiveExpectedType, { allowConversion });
    });
  }
  ...
  if (typeof variables === 'object') {
    ...
    Object.keys(variables).forEach((category) => {
      if (Array.isArray(variables[category])) {
        filteredVariables[category] = variables[category].filter((variable) => {
          const varType = variable.returnType || variable.type || 'any';
          return isTypeCompatible(varType, effectiveExpectedType, { allowConversion });
        });
      } else {
        filteredVariables[category] = variables[category];
      }
    });
...
export const filterFunctionsByReturnType = (functions, expectedType) => {
  if (!functions || !expectedType || expectedType === 'any') {
    return functions;
  }
  ...
  if (Array.isArray(functions)) {
    return functions.filter((fn) => {
      ...
      return isTypeCompatible(returnType, expectedType, { allowConversion: false });
    });
  }
...
export const filterDataBlocksByType = (data, expectedType, functionContext = null) => {
  if (!data || !expectedType || expectedType === 'any') {
    return data;
  }
  ...
  if (functionContext && functionContext.currentFunction) {
    ...
    if (funcDef && funcDef.args && funcDef.args[currentArgIndex]) {
      const argDef = funcDef.args[currentArgIndex];
      effectiveExpectedType = argDef.type || expectedType;
  ...
  Object.keys(data).forEach((category) => {
    ...
    if (Array.isArray(categoryData)) {
      filteredData[category] = categoryData.filter((item) => {
        const itemType = item.returnType || item.type || 'any';
        return isTypeCompatible(itemType, effectiveExpectedType, { allowConversion });
      });
      return;
    }
    ...
    Object.keys(categoryData).forEach((subCategory) => {
      if (Array.isArray(categoryData[subCategory])) {
        if (subCategory === 'FUNCTIONS') {
          filteredCategoryData[subCategory] = categoryData[subCategory];
        } else {
          filteredCategoryData[subCategory] = categoryData[subCategory].filter((item) => {
            const itemType = item.returnType || item.type || 'any';
            return isTypeCompatible(itemType, effectiveExpectedType, { allowConversion });
          });
        }
    ...
    filteredData[category] = filteredCategoryData;
  });
  
  return filteredData;
};

export const getFunctionContextFromContent = (content, cursorPosition, functionRegistry) => {
  if (!content || content.length === 0 || !functionRegistry) {
    return null;
  }
  ...
  for (let i = 0; i < content.length && i < cursorPosition; i++) {
    const item = content[i];
    
    if (item.subCategory === 'FUNCTIONS' || item.type === 'FUNCTIONS') {
      functionStack.push({
        name: item.value,
        argIndex: 0,
      });
    } else if (item.value === '(' || item.subType === '(') {
      openParens++;
    } else if (item.value === ')' || item.subType === ')') {
      openParens--;
  ...
  if (functionStack.length > 0) {
    const currentCtx = functionStack[functionStack.length - 1];
    return {
      currentFunction: currentCtx.name,
      currentArgIndex: currentCtx.argIndex,
      functionRegistry,
    };
  }
