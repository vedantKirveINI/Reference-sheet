...
import { FIND_ONE_V2_NODE, MYSQL_FIND_ONE_RECORD_NODE, POSTGRES_FIND_ONE_RECORD_NODE, TABS, THEME } from "./constants";
import { useFindOneV2State } from "./hooks/useFindOneV2State";
...
const FindOneV2 = forwardRef(
  (
    ...
    const state = useFindOneV2State(data, workspaceId, projectId);
    ...
    const selectedNode = useMemo(() => {
      if (effectiveDatabaseType === DATABASE_TYPES.MYSQL) {
        return MYSQL_FIND_ONE_RECORD_NODE;
      }
      if (effectiveDatabaseType === DATABASE_TYPES.POSTGRESQL) {
        return POSTGRES_FIND_ONE_RECORD_NODE;
      }
      return FIND_ONE_V2_NODE; // fallback
    ...
    const getFooterGuidance = () => {
      ...
      if (activeTab === TABS.CONFIGURE && !state.validation.isValid) {
        return "Complete the configuration to continue";
      }
        ...
        case TABS.TEST:
          return (
            <CommonTestModuleV3
              ...
              parentId={parentId}
              variables={variables}
              node={nodeData || FIND_ONE_V2_NODE}
    ...
    return (
      <WizardDrawer
        ...
        subtitle={FIND_ONE_V2_NODE.description}
        tabs={tabs}
        activeTab={activeTab}
...
FindOneV2.displayName = "FindOneV2";

export default FindOneV2;
