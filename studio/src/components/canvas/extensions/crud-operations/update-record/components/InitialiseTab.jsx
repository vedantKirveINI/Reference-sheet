...
import { Database, Table, Check, Zap, Link, Pencil, Loader2 } from "lucide-react";
  ...
  return (
    <div className="space-y-8">
      <div className="text-center space-y-4">
        ...
        >
          <Pencil className="w-8 h-8" style={{ color: themeColor }} />
        </div>
        <div>
          <h2 className="text-2xl font-semibold text-gray-900">Update Record</h2>
          <p className="text-gray-500 mt-2 max-w-md mx-auto">
            Modify existing rows in your {dbConfig.name} database
          </p>
        </div>
      </div>
      ...
      {connection && table && (
        ...
        >
          <p className="text-sm font-medium" style={{ color: themeColor }}>
            Connection and table selected. Click Continue to configure your update.
          </p>
        </div>
      )}
        ...
        <div className="bg-gray-50 rounded-xl p-4 space-y-3">
          ...
          <ul className="space-y-2 text-sm text-gray-600">
            <li className="flex items-start gap-2">
              ...
              <span><strong>Modify existing data</strong> — Change values in specific rows based on conditions</span>
            </li>
            <li className="flex items-start gap-2">
              <span style={{ color: themeColor }} className="mt-0.5">•</span>
              <span><strong>Update status fields</strong> — Mark orders as complete, toggle flags, etc.</span>
            </li>
            <li className="flex items-start gap-2">
              <span style={{ color: themeColor }} className="mt-0.5">•</span>
              <span><strong>Sync external data</strong> — Keep your database in sync with other systems</span>
            </li>
          </ul>
        </div>
      )}
