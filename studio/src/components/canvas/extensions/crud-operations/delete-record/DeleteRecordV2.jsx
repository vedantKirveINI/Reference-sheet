...
import { DELETE_V2_NODE, MYSQL_DELETE_RECORD_NODE, POSTGRES_DELETE_RECORD_NODE, TABS, THEME } from "./constants";
import { useDeleteV2State } from "./hooks/useDeleteV2State";
...
const DeleteRecordV2 = forwardRef(
  (
    {
      canvasRef,
      annotation,
    ...
    const state = useDeleteV2State(data, workspaceId, projectId);
    ...
    const selectedNode = useMemo(() => {
      if (effectiveDatabaseType === DATABASE_TYPES.MYSQL) {
        return MYSQL_DELETE_RECORD_NODE;
      }
      if (effectiveDatabaseType === DATABASE_TYPES.POSTGRESQL) {
        return POSTGRES_DELETE_RECORD_NODE;
      }
      return DELETE_V2_NODE; // fallback
    ...
    const getFooterGuidance = () => {
      if (activeTab === TABS.INITIALISE && !state.hasInitialised) {
        return "Select a database connection and table to continue";
      }
      if (activeTab === TABS.CONFIGURE && !state.validation.isValid) {
        return "Configure WHERE clause to specify records to delete";
      }
      if (activeTab === TABS.TEST) {
        return "Run a test to validate the delete operation";
      }
      return null;
    };
    ...
    return (
      <WizardDrawer
        ...
        subtitle={DELETE_V2_NODE.description}
        tabs={tabs}
        activeTab={activeTab}
...
DeleteRecordV2.displayName = "DeleteRecordV2";

export default DeleteRecordV2;
