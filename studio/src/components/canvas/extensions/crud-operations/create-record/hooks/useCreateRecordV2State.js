...
export const useCreateRecordV2State = (initialData = {}, workspaceId, projectId) => {
  ...
  const refreshConnection = useCallback(async (databaseType) => {
    ...
    try {
      ...
      console.error("[useCreateRecordV2State] Error refreshing connection:", error);
  ...
  const validation = useMemo(() => {
    ...
    const requiredFields = schemaFields.filter(f => f.field_indicator === "REQUIRED");
    const hasMissingRequired = requiredFields.some((field) => {
      const recordItem = record.find(r => r.key === field.name);
      return !recordItem?.value?.blocks || recordItem.value.blocks.length === 0;
    });
  ...
  }, [connection, table, record, schemaFields]);
  ...
  }, [connection, table, record, outputSchema]);
