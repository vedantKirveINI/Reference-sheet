...
import { Database, Table, Check, Zap, Link } from "lucide-react";
  ...
  return (
    <div className="space-y-8">
      <div className="text-center space-y-4">
        ...
        >
          <Database className="w-8 h-8" style={{ color: themeColor }} />
        </div>
        <div>
          <h2 className="text-2xl font-semibold text-gray-900">Create Record</h2>
          <p className="text-gray-500 mt-2 max-w-md mx-auto">
            Insert new rows into your {dbConfig.name} database
          </p>
        </div>
      </div>
          ...
          {connection ? (
            <div className="flex items-center gap-2">
              ...
              {table && <Button
                ...
                size="icon"
                onClick={() => {
                  refreshConnection(databaseType)
                }}
      ...
      {connection && table && (
        ...
        >
          <p className="text-sm font-medium" style={{ color: themeColor }}>
            Connection and table selected. Click Continue to map your fields.
          </p>
        </div>
      )}
        ...
        <div className="bg-gray-50 rounded-xl p-4 space-y-3">
          ...
          <ul className="space-y-2 text-sm text-gray-600">
            <li className="flex items-start gap-2">
              ...
              <span><strong>Save form submissions</strong> — Store user input directly in your database</span>
            </li>
            <li className="flex items-start gap-2">
              <span style={{ color: themeColor }} className="mt-0.5">•</span>
            ...
            <li className="flex items-start gap-2">
              <span style={{ color: themeColor }} className="mt-0.5">•</span>
              <span><strong>Store workflow outputs</strong> — Persist data from automations for later use</span>
            </li>
          </ul>
        </div>
      )}
