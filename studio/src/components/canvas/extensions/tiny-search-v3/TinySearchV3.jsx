...
import { TINY_SEARCH_V3_NODE, TABS, THEME } from "./constants";
...
const TinySearchV3 = forwardRef(
  (
    ...
    const state = useSearchState(data);
    ...
    const getPrimaryActionLabel = () => {
      ...
      if (activeTab === TABS.CONFIGURE) return "Test Search";
      return "Save & Close";
    };
    ...
    const getFooterGuidance = () => {
      if (activeTab === TABS.INITIALISE) {
        ...
        return "Click Continue to configure your search parameters";
      }
      if (activeTab === TABS.CONFIGURE) {
        ...
        if (hasTouchedErrors) {
          if (touchedErrors.searchQuery) return touchedErrors.searchQuery;
          return "Please fix the errors above";
        }
        return "Configure your search and click Test Search to continue";
      }
      if (activeTab === TABS.TEST) {
        return "Review the results and click Save & Close when done";
      }
      return "";
    };
        ...
        case TABS.TEST:
          return (
            <CommonTestModuleV3
              ...
              parentId={parentId}
              variables={variables}
              node={nodeData || TINY_SEARCH_V3_NODE}
              onTestComplete={handleTestComplete}
              onProcessingChange={setIsTestProcessing}
              theme={{
                primaryColor: TINY_SEARCH_V3_NODE.light,
                secondaryColor: TINY_SEARCH_V3_NODE.dark,
              }}
            />
          );
        default:
    ...
    return (
      <>
        <WizardDrawer
          ref={drawerRef}
          open={open}
          icon={<img src={TINY_SEARCH_V3_NODE._src} alt="TinySearch" className="w-5 h-5" />}
          title={state.name || "Tiny Search"}
          tabs={tabs}
          activeTab={activeTab}
...
TinySearchV3.displayName = "TinySearchV3";

export default TinySearchV3;
