...
import { TINYGPT_LEARNING_V2_NODE, TABS, THEME } from "./constants";
...
const TinyGPTLearningV2 = forwardRef(
  (
    {
      canvasRef,
      annotation,
    ...
    const state = useLearningState(data);
    ...
    const getFooterGuidance = () => {
      if (activeTab === TABS.INITIALISE) {
        if (!state.selectedTemplateId && !state.isFromScratch) {
          return "Choose an educational format or start from scratch";
        }
        return "Click Continue to configure your educational content";
      }
      if (activeTab === TABS.CONFIGURE) {
        if (!state.validation.isValid) {
          const errors = state.validation.errors;
          if (errors.topic) return errors.topic;
          if (errors.output) return errors.output;
          return "Fill in all required fields to continue";
        }
        return "Define the topic and learning objectives";
      }
      if (activeTab === TABS.TEST) {
        return "Run a test to preview the educational content";
      }
      return null;
    };
        ...
        case TABS.TEST:
          return (
            <CommonTestModuleV3
              ...
              parentId={parentId}
              variables={variables}
              node={nodeData || TINYGPT_LEARNING_V2_NODE}
    ...
    return (
      <>
        <WizardDrawer
          ...
          title={data.name || "Tiny GPT Learning"}
          subtitle="Create educational content and explanations"
          tabs={tabs}
          activeTab={activeTab}
...
TinyGPTLearningV2.displayName = "TinyGPTLearningV2";

export default TinyGPTLearningV2;
