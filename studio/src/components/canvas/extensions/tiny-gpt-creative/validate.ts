...
export function validateGPTCreative(context: NodeValidationContext) {
  ...
  if (!hasSystemPrompt) {
    issues.push(required(
      'persona',
      'System Prompt',
      'Define a creative persona or style guide'
    ));
  }
...
registerValidator('GPT_CREATIVE', validateGPTCreative);
