...
import TinyGPTTestModule from "./components/TinyGPTTestModule";
import UnsavedChangesPrompt from "../common-components/UnsavedChangesPrompt";
import { TINYGPT_NODE, TINYGPT_TEMPLATES, TABS, THEME } from "./constants";
import { useTinyGPTState } from "./hooks/useTinyGPTState";
...
const TinyGPT = forwardRef(
  (
    ...
    const state = useTinyGPTState(data);
    ...
    const getFooterGuidance = () => {
      ...
      if (activeTab === TABS.CONFIGURE) {
        ...
        if (hasTouchedErrors) {
          if (touchedErrors.systemPrompt) return touchedErrors.systemPrompt;
          if (touchedErrors.prompt) return touchedErrors.prompt;
          if (touchedErrors.output) return touchedErrors.output;
    ...
    return (
      <>
        <WizardDrawer
          ref={drawerRef}
          open={open}
          icon={<img src="https://cdn-v1.tinycommand.com/1234567890/1770458055954/tiny%20gpt.svg" alt="TinyGPT" className="w-5 h-5" />}
