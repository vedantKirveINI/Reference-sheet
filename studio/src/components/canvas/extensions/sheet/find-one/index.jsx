...
import { FIND_ONE_SHEET_RECORD_NODE, TABS, THEME } from "./constants";
import { useFindOneState } from "./hooks/useFindOneState";
...
const FindOneRecord = forwardRef(
  (
    ...
    const state = useFindOneState(data);
    ...
    // Wrap handlers to sync both useSheet and useFindOneState
    ...
    const fieldConfig = useMemo(() => buildFieldConfig({ sortedFields, nodeName: 'FindOne' }), [sortedFields]);
    ...
    return (
      <WizardDrawer
        ref={drawerRef}
        open={open}
        icon={<img src={FIND_ONE_SHEET_RECORD_NODE._src} alt="Find One Record" className="w-5 h-5" />}
        title={nodeData?.name || FIND_ONE_SHEET_RECORD_NODE.name}
        showEditTitle={true}
        onTitleChange={(newTitle) => { state.updateState({ name: newTitle }); onUpdateTitle({ name: newTitle }); }}
        subtitle={FIND_ONE_SHEET_RECORD_NODE.description}
        tabs={tabs}
        activeTab={activeTab}
...
FindOneRecord.displayName = "FindOneRecord";

export default FindOneRecord;
export { FIND_ONE_SHEET_RECORD_NODE } from "./constants";
