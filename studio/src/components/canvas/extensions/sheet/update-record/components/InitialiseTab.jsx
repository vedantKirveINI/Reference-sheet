...
import tinyTablesUpdateIcon from "@/components/canvas/assets/extensions/tiny-tables-update.svg";
...
const InitialiseTab = ({
  ...
  const getDynamicTemplates = useMemo(() => {
    const templates = [
      {
        id: "update-record",
        name: "Update Record",
        description: "Modify existing row data in your sheet",
        icon: "Edit",
      },
    ];
    ...
    const statusField = sortedFields.find(f =>
      ['status', 'state', 'stage', 'progress'].some(kw =>
    ...
    if (statusField) {
      templates.push({
        id: "update-status",
        name: `Update ${statusField.name}`,
        description: `Change ${statusField.name.toLowerCase()} value for a record`,
        icon: "RefreshCw",
        fieldId: statusField.id,
      });
    }

    const completedField = sortedFields.find(f =>
      ['complete', 'done', 'finished', 'checked'].some(kw =>
        f.name.toLowerCase().includes(kw)
    ...
    if (completedField) {
      templates.push({
        id: "mark-complete",
        name: `Mark as Complete (${completedField.name})`,
        description: `Set ${completedField.name.toLowerCase()} to mark items done`,
        icon: "CheckCircle",
        fieldId: completedField.id,
      });
    }
    ...
    <div className="space-y-8">
      <div className="text-center space-y-4">
        <div className="mx-auto w-16 h-16 bg-[#22C55E]/10 rounded-2xl flex items-center justify-center">
          <img src={tinyTablesUpdateIcon} alt="Update Record" className="w-8 h-8" />
        </div>
        <div>
          <h2 className="text-2xl font-semibold text-gray-900">Update Record</h2>
          <p className="text-gray-500 mt-2 max-w-md mx-auto">
            Update existing rows in your sheet
          </p>
        </div>
      </div>
        ...
        <>
          <div className="bg-gray-50 rounded-xl p-4 space-y-3">
            <h3 className="font-medium text-gray-900 flex items-center gap-2">
              ...
              When to use Update Record
            </h3>
            <ul className="space-y-2 text-sm text-gray-600">
              <li className="flex items-start gap-2">
                <span className="text-[#22C55E] mt-0.5">•</span>
                <span><strong>Update status</strong> — Change order status, task completion, etc.</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-[#22C55E] mt-0.5">•</span>
                <span><strong>Modify records</strong> — Update customer details, inventory, or any data</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-[#22C55E] mt-0.5">•</span>
                <span><strong>Sync data</strong> — Keep data synchronized across systems</span>
              </li>
            </ul>
          </div>
          ...
          >
            ...
            <p className="text-sm text-gray-500 mt-1">Configure your own update settings</p>
          </button>
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <div className={cn(
                        ...
                        <img src={tinyTablesUpdateIcon} alt={template.name} className="w-5 h-5" />
                      </div>
