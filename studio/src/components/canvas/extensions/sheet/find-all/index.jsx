...
import { FIND_ALL_SHEET_RECORD_NODE, TABS, THEME } from "./constants";
import { useFindAllState } from "./hooks/useFindAllState";
...
const FindAllRecord = forwardRef(
  (
    ...
    const state = useFindAllState(data);
    ...
    // Wrap handlers to sync both useSheet and useFindAllState
    ...
    const fieldConfig = useMemo(() => buildFieldConfig({ sortedFields, nodeName: 'FindAll' }), [sortedFields]);
    ...
    return (
      <WizardDrawer
        ref={drawerRef}
        open={open}
        icon={<img src={FIND_ALL_SHEET_RECORD_NODE._src} alt="Find All Records" className="w-5 h-5" />}
        title={nodeData?.name || FIND_ALL_SHEET_RECORD_NODE.name}
        showEditTitle={true}
        onTitleChange={(newTitle) => { state.updateState({ name: newTitle }); onUpdateTitle({ name: newTitle }); }}
        subtitle={FIND_ALL_SHEET_RECORD_NODE.description}
        tabs={tabs}
        activeTab={activeTab}
...
FindAllRecord.displayName = "FindAllRecord";

export default FindAllRecord;
export { FIND_ALL_SHEET_RECORD_NODE } from "./constants";
