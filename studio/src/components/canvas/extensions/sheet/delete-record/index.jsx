...
import { DELETE_SHEET_RECORD_NODE, TABS, THEME } from "./constants";
import { useDeleteRecordState } from "./hooks/useDeleteRecordState";
...
const DeleteRecord = forwardRef(
  (
    ...
    const state = useDeleteRecordState(data);
    ...
    // Wrap handlers to sync both useSheet and useDeleteRecordState
    ...
    }, [hasUnsavedChanges, onSave, onClose, state, sheet, table]);
    ...
    const handleSaveAndClose = useCallback(() => {
      ...
      onClose();
    }, [onSave, onClose, state, sheet, table]);
    ...
    }, [activeTab, state.hasInitialised, state.validation.isValid, onSave, onClose, state, sheet, table]);
    ...
    const getFooterGuidance = () => {
      ...
      if (activeTab === TABS.CONFIGURE && !state.validation.isValid) {
        return "Configure the delete settings to continue";
      }
    ...
    const fieldConfig = useMemo(() => buildFieldConfig({ sortedFields, nodeName: 'DeleteRecord' }), [sortedFields]);
    ...
    return (
      <WizardDrawer
        ref={drawerRef}
        open={open}
        icon={<img src={DELETE_SHEET_RECORD_NODE._src} alt="Delete Record" className="w-5 h-5" />}
        title={nodeData?.name || DELETE_SHEET_RECORD_NODE.name}
        showEditTitle={true}
        onTitleChange={(newTitle) => { state.updateState({ name: newTitle }); onUpdateTitle({ name: newTitle }); }}
        subtitle={DELETE_SHEET_RECORD_NODE.description}
        tabs={tabs}
        activeTab={activeTab}
...
export { DELETE_SHEET_RECORD_NODE } from "./constants";
