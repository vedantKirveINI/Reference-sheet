...
import { UPDATE_SHEET_RECORDS_NODE, TABS, THEME } from "./constants";
import { useUpdateRecordsState } from "./hooks/useUpdateRecordsState";
...
const UpdateRecords = forwardRef(
  (
    {
      canvasRef,
      annotation,
    ...
    const state = useUpdateRecordsState(data);
    ...
    const fieldConfig = useMemo(() => buildFieldConfig({ sortedFields, nodeName: 'UpdateRecords' }), [sortedFields]);
        ...
        case TABS.TEST:
          return (
            <CommonTestModuleV3
              ...
              parentId={parentId}
              variables={variables}
              node={nodeData || UPDATE_SHEET_RECORDS_NODE}
    ...
    return (
      <WizardDrawer
        ref={drawerRef}
        open={open}
        icon={<img src={UPDATE_SHEET_RECORDS_NODE._src} alt="Update Records" className="w-5 h-5" />}
        title={nodeData?.name || UPDATE_SHEET_RECORDS_NODE.name}
        showEditTitle={true}
        onTitleChange={(newTitle) => { state.updateState({ name: newTitle }); onUpdateTitle({ name: newTitle }); }}
        subtitle={UPDATE_SHEET_RECORDS_NODE.description}
        tabs={tabs}
        activeTab={activeTab}
...
UpdateRecords.displayName = "UpdateRecords";

export default UpdateRecords;
export { UPDATE_SHEET_RECORDS_NODE } from "./constants";
