...
import { Search, SlidersHorizontal, Settings, Play } from "lucide-react";
...
import { GPT_ANALYZER_NODE, TABS, THEME } from "./constants";
...
const GPTAnalyzer = forwardRef(
  (
    ...
    const getFooterGuidance = () => {
      if (activeTab === TABS.INITIALISE) {
        if (!state.selectedTemplateId) {
          return "Select a template or start from scratch to define your analysis";
        }
      ...
      if (activeTab === TABS.CONFIGURE) {
        ...
        return "Configure your analysis prompt and output format, then test it";
      }
      if (activeTab === TABS.TEST) {
        return "Run a test to validate the analysis before saving";
      }
      return null;
    };
        ...
        case TABS.TEST:
          return (
            <CommonTestModuleV3
              ...
              parentId={parentId}
              variables={variables}
              node={nodeData || GPT_ANALYZER_NODE}
    ...
    return (
      <>
        <WizardDrawer
          ...
          subtitle="Analyze data and extract insights"
          tabs={tabs}
          activeTab={activeTab}
...
GPTAnalyzer.displayName = "GPTAnalyzer";
