...
import { BarChart3, ThumbsUp, Tags, Search, Target } from "lucide-react";
import { ANALYZER_TEMPLATE_PRESETS } from "../templates/presets";
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "border-2",
        isSelected
          ? "border-violet-500 shadow-[0_2px_12px_rgba(124,58,237,0.25)]"
    ...
    <div className="space-y-8 p-6">
      ...
      <div className="bg-violet-50 rounded-xl p-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          <UseCaseItem icon={ThumbsUp} text="Sentiment analysis" />
          <UseCaseItem icon={Tags} text="Content classification" />
          <UseCaseItem icon={Search} text="Entity extraction" />
          <UseCaseItem icon={Target} text="Intent detection" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {ANALYZER_TEMPLATE_PRESETS.map((template) => (
          <TemplateCard
            key={template.id}
            template={template}
