...
export function SearchBar({
  ...
  // Always render the trigger button in the toolbar
  const triggerButton = (
    <Button
      variant="ghost"
      size="xs"
      data-island-btn
      className={cn("font-normal shrink-0", isOpen && "bg-accent/40")}
      style={triggerStyle}
  ...
  // Floating search panel — portaled to document.body because ancestors
  // (main overflow-hidden + Radix ScrollArea transform) break position:fixed.
  const searchPanel = isOpen
    ? createPortal(
        <div
          className={cn(
            "fixed z-[9999] w-[26rem]",
            "border border-border/60 bg-background shadow-lg rounded-lg",
          )}
          style={{ top: 'calc(2.75rem + 2.625rem + 1.5rem + 0.25rem)', right: '1rem' }}
        >
