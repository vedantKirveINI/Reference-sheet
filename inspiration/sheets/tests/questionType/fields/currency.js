...
export async function currencyCreateField({ page }) {
	const colBtn = page
	...
	await page.getByPlaceholder("Enter field name").fill("Currency");
	...
	// await page.getByPlaceholder("Enter default value (optional)").fill("INR");
	...
	await page.getByRole("button", { name: "SAVE" }).click();
	// await expect(
	// 	page.getByText("Only numeric values are allowed"),
	// ).toBeVisible();
	// await page.getByPlaceholder("Enter default value (optional)").click();
	...
	const colName = page
		...
		.getByTestId("column-currency-5")
		.locator("span");

	await expect(colName).toHaveText("Currency");
}
