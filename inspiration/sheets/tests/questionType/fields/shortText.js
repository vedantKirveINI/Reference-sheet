import { expect } from "@playwright/test";

export async function shortTextFieldCreation({ page }) {
	const colBtn = page
		.getByRole("columnheader", { name: "icon Name" })
		.locator("button");

	await expect(colBtn).toBeVisible();

	await colBtn.click();

	await page.getByLabel("Insert column right").click();

	await page.waitForTimeout(500);

	await page.getByRole("button", { name: "SAVE" }).click();

	await expect(page.getByText("Please enter a field name")).toBeVisible();

	await page.getByPlaceholder("Enter field name").fill("Short Text");

	await page.getByTestId("ods-autocomplete").locator("input").nth(1).click();
	await page.getByRole("option", { name: "Short Text" }).click();
	await page
		.getByPlaceholder("Enter description (optional)")
		.fill("Short Text: description");
	// await page
	// 	.getByPlaceholder("Enter default value (optional)")
	// 	.fill("Short Text: default Value");
	await page.getByRole("button", { name: "SAVE" }).click();

	const response = await page.waitForResponse((resp) => {
		return resp.url().includes("/field/create_field");
	});
	expect(response.status()).toBeGreaterThanOrEqual(200);
	expect(response.status()).toBeLessThan(300);

	const colName = page
		.locator(".ht_clone_top")
		.getByTestId("column-short-text-1")
		.locator("span");

	await expect(colName).toHaveText("Short Text");
}
