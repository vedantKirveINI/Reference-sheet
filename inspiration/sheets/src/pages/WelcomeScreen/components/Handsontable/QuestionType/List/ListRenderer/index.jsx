...
const ListRenderer = (props) => {
	...
	return (
		<div data-column-index={col} className={`${styles.list_container}`}>
			{/* Visible chips */}
			<div
			...
			>
				{visibleChips.map((chipValue, index) => {
					const bgColor = getChipsColor({ index, type: "mcq" }); // Using mcq colors for now

					return (
						<span
							key={`${index}_${chipValue}`}
							className={`${styles.list_dropdown_chip} ${isWrapped ? styles.full_width_chip : ""}`}
							style={{
				...
				{limitValue && !isWrapped && (
					<span className={styles.limit_text} data-list-limit-chip>
						{limitValue}
					</span>
				)}
			</div>
			...
			<div className={styles.hidden_chips_container}>
				{hiddenChips.map((chipValue, index) => {
					...
					const bgColor = getChipsColor({
						index: originalIndex,
						type: "mcq", // Using mcq colors for now
					});
					return (
						<span
							data-hidden-list-chip
							key={`hidden_${index}_${chipValue}`}
							className={styles.list_dropdown_chip}
							style={{
								backgroundColor: bgColor,
								visibility: "hidden",
							}}
