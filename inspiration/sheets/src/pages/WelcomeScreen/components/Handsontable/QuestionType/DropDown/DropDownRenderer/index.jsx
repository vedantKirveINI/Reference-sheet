...
const DropDownRenderer = (props) => {
	...
	const { limitValue = 0, visibleChips = [] } = useChipWidths({
		selectionValues: getOptionLabel({ optionsList: parsedValue }),
		availableHeight,
		availableWidth,
		...
		>
			...
			>
				{visibleChips.map((chipValue, index) => {
					...
					return (
						<span
							...
							className={`${styles.dropdown_dynamic_chip} ${isWrapped ? styles.full_width_chip : ""}`}
							style={{
				...
				{limitValue && !isWrapped && (
					<span
						...
						data-dropdown-dynamic-limit-chip
					>
						{limitValue}
					</span>
				)}
			...
			<div className={styles.hidden_chips_container}>
				{hiddenChips.map((chipValue, index) => {
					...
					return (
						<span
							data-hidden-dropdown-dynamic-chip
							key={`hidden_${index}_${chipValue}`}
							className={styles.dropdown_dynamic_chip}
							style={{
								backgroundColor: bgColor,
								visibility: "hidden",
							}}
