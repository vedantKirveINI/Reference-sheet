    ...
    try {
      ...
      for (const input of (task_obj?.inputs || [])) {
        ...
        if(input?.isValueMode === true && _.isArray(input?.value)) {
          ...
          field_info.data = flow_utility_instance.resolveValue(state, (input?.dbFieldName || input?.key), input?.value, input?.type, undefined, null)?.value;
        }
      ...
      let req_options = {
        ...
        method: "POST",
        url: `${sheet_bff?.url}${sheet_bff?.update_record_sub_sheet_v2}`,
        headers: {
