...
import { base64Decode, base64Encode } from "@src/module/ods";
import DataBlock from "../components/data-block/DataBlock.jsx";
import { FUNCTIONS } from "../constants/types.jsx";
...
/**
 * Detects if cursor is positioned inside function brackets
 * Checks if cursor is between the last opening "(" and its matching closing ")"
 * @param {HTMLElement} contentEditableEl - The contentEditable element
 * @param {Range} range - The current selection range
 * @returns {boolean} - True if cursor is inside function brackets
 */
export const isInsideFunctionBrackets = (contentEditableEl, range) => {
  if (!contentEditableEl || !range) return false;
  ...
  // Helper to check if element is an opening bracket (function block or "(" operator)
  const isOpeningBracket = (element) => {
    const blockData = getBlockData(element);
    if (!blockData) return false;

    // Function blocks implicitly have "(" (they display with "(")
  ...
  // Helper to check if element is a closing bracket operator
  const isClosingBracket = (element) => {
    const blockData = getBlockData(element);
    return blockData && blockData.value === ")";
  };

  // Walk backwards from cursor to find the last opening bracket
  let currentNode = range.startContainer;
  let offset = range.startOffset;
  let nodeToCheck;

  // Start from the node before cursor
  if (currentNode.nodeType === Node.TEXT_NODE) {
    nodeToCheck = currentNode.previousSibling;
  } else if (currentNode.nodeType === Node.ELEMENT_NODE) {
    if (offset > 0 && currentNode.childNodes[offset - 1]) {
      nodeToCheck = currentNode.childNodes[offset - 1];
    } else {
      nodeToCheck = currentNode.previousSibling;
    }
  } else {
  ...
  let lastOpeningBracketNode = null;

  // Traverse backwards to find the last opening bracket
  // Continue past semicolons and other blocks until we find an opening bracket
  while (nodeToCheck) {
    if (isOpeningBracket(nodeToCheck)) {
      lastOpeningBracketNode = nodeToCheck;
      break;
    }
    // Continue searching backwards, don't stop at semicolons or other blocks
    nodeToCheck = nodeToCheck.previousSibling;
  }

  if (!lastOpeningBracketNode) {
    return false;
  }

  // Now walk forwards from cursor to find the matching closing bracket
  let forwardNode;
  if (currentNode.nodeType === Node.TEXT_NODE) {
    forwardNode = currentNode.nextSibling;
  } else if (currentNode.nodeType === Node.ELEMENT_NODE) {
    if (offset < currentNode.childNodes.length) {
      forwardNode = currentNode.childNodes[offset];
    } else {
      forwardNode = currentNode.nextSibling;
    }
  } else {
  ...
  // Count bracket depth to handle nested functions
  let bracketDepth = 0;
  while (forwardNode) {
    if (isOpeningBracket(forwardNode)) {
      bracketDepth++; // Nested opening bracket increases depth
    } else if (isClosingBracket(forwardNode)) {
      if (bracketDepth === 0) {
        // Found matching closing bracket for our opening bracket
        return true;
      }
      bracketDepth--; // Close nested bracket
    }
    forwardNode = forwardNode.nextSibling;
  }

  // If we found an opening bracket but no closing bracket, assume we're still inside
  // (the function might be incomplete)
  return true;
};

/**
 * Returns the semicolon operator block from otherData
 * @returns {Object} - Semicolon operator block
 */
export const getSemicolonOperator = () => {
