...
export const getMultiQuestionErrors = (question: Record<string, any>) => {
  ...
  const isDefaultValueInOptions = defaultValue?.some((option: string) =>
    options?.includes(option)
  );

  if (options?.length <= 0) {
    errors.push(NODE_ERROR_MESSAGES.MCQ.optionsError);
    settingsErrors.optionsError = NODE_ERROR_MESSAGES.MCQ.optionsError;
  } else {
    if (settingsErrors.optionsError) {
      settingsErrors.optionsError = "";
      errors = errors.filter(
        (error) => error !== NODE_ERROR_MESSAGES.MCQ.optionsError
      );
    }
  }

  if (!isDefaultValueInOptions && defaultValue?.length) {
    errors.push(NODE_ERROR_MESSAGES.MCQ.defaultValueError);
    settingsErrors.defaultValueError =
      NODE_ERROR_MESSAGES.MCQ.defaultValueError;
  } else if (isDefaultValueInOptions) {
    ...
    if (settingsErrors.defaultValueError) {
      ...
      errors = errors.filter(
        (error) => error !== NODE_ERROR_MESSAGES.MCQ.defaultValueError
      );
    }
  }
