...
// =============================================================================
// Cache the Fuse instance to avoid recreating it on every search
let cachedFuseInstance = null;
let cachedCandidates = null;
let cachedCandidatesLength = 0;
...
export const INTENT_KEYWORDS = {
  ...
  "check condition": ["IFELSE_V2", "IF_ELSE", "IF_ELSE_V2"],
  "loop through": ["FOR_EACH", "REPEAT", "LOOP_UNTIL", "ITERATOR"],
  iterate: ["ITERATOR", "FOR_EACH"],

  loop: ["FOR_EACH", "REPEAT", "LOOP_UNTIL"],
  repeat: ["REPEAT", "FOR_EACH", "LOOP_UNTIL"],
  "for each": ["FOR_EACH", "ITERATOR"],
  "loop until": ["LOOP_UNTIL", "REPEAT", "FOR_EACH"],
...
export const CONTEXTUAL_SUGGESTIONS = {
  HTTP: ["TRANSFORMER", "IFELSE_V2", "ITERATOR"],
  SEND_EMAIL: ["DELAY", "IFELSE_V2"],
  IFELSE_V2: ["SEND_EMAIL", "HTTP", "UPSERT"],
  IF_ELSE: ["SEND_EMAIL", "HTTP", "UPSERT"],
  ITERATOR: ["AGGREGATOR", "UPSERT", "SEND_EMAIL"],
  TRANSFORMER: ["IFELSE_V2", "HTTP", "UPSERT"],
  FIND_ALL: ["ITERATOR", "TRANSFORMER", "IFELSE_V2"],
  FIND_ONE: ["IFELSE_V2", "TRANSFORMER", "HTTP"],
  FORM_TRIGGER: ["IFELSE_V2", "SEND_EMAIL", "UPSERT"],
  TABLE_TRIGGER: ["IFELSE_V2", "SEND_EMAIL", "HTTP"],
...
export const SIMILAR_TERMS = {
  ...
  loop: ["for each", "repeat", "loop until", "iterate"],
...
/**
 * Enhances candidates with searchable description data from nodeDescriptions.
 * @param {Array} candidates - Nodes to enhance
 * @returns {Array} Candidates with _searchMeta field added
 */
function enhanceCandidatesWithDescriptions(candidates) {
  return candidates.map((node) => {
    const desc = getNodeDescription(node);
    const descriptionFromMapper = getNodeDescriptionText(node.type) || "";
    const description = node.description ?? descriptionFromMapper;
    const enhanced = {
      ...node,
      description,
    };
    if (desc) {
      enhanced._searchMeta = {
        tagline: desc.tagline || "",
        descriptionText: desc.description?.replace(/\*\*/g, "") || "",
        keywords: desc.keywords?.join(" ") || "",
        useCaseText: desc.useCases?.map((uc) => uc.text).join(" ") || "",
      };
    } else if (descriptionFromMapper) {
      enhanced._searchMeta = {
        tagline: "",
        descriptionText: descriptionFromMapper,
        keywords: "",
        useCaseText: "",
      };
    }
 ...
 * Includes node description keywords with lower weight for broader matching.
 * Uses cached Fuse instance when candidates haven't changed for better performance.
...
export function applyFuzzySearch(candidates, searchText) {
  ...
  const fuseOptions = {
    keys: [
      ...
      { name: "meta.search_keys", weight: 0.25 },
      { name: "parentName", weight: 0.2 },
      { name: "_searchMeta.tagline", weight: 0.15 },
      { name: "_searchMeta.keywords", weight: 0.12 },
      { name: "_searchMeta.descriptionText", weight: 0.08 },
      { name: "_searchMeta.useCaseText", weight: 0.06 },
    ],
    includeScore: true,
    threshold: 0.5,
  ...
  const candidatesChanged =
    ...
    candidates.length !== cachedCandidatesLength ||
    cachedFuseInstance === null;

  if (candidatesChanged) {
    const enhancedCandidates = enhanceCandidatesWithDescriptions(candidates);
    cachedFuseInstance = new Fuse(enhancedCandidates, fuseOptions);
    cachedCandidates = candidates;
    cachedCandidatesLength = candidates.length;
  }

  return cachedFuseInstance.search(searchText);
}
...
export function applyStaticSignals(candidates, searchText, context) {
  ...
  if (intentTypes.length > 0) {
    ...
    const fuseIds = new Set(
      fuseResults.map(
        (r) => r.item.type + (r.item.isChildAction ? r.item.name : ""),
      ),
    );
...
export function runSuggestionPipeline({
  ...
  const filteredNodes = applyStaticSignals(allNodesWithChildren, searchText, {
