...
function NodeCard({ node, defaultExpanded }) {
  ...
  return (
    <div className={cn("rounded-lg border overflow-hidden transition-shadow hover:shadow-md", isFailed && "border-destructive/50")}>
      <div
      ...
      >
        ...
        <div className="flex shrink-0 items-center gap-2">
          {duration && <span className="text-[0.625rem] tabular-nums text-muted-foreground">{duration}</span>}
          <span className={cn("rounded px-2 py-0.5 text-[0.625rem] font-semibold", isFailed ? "bg-destructive/10 text-destructive" : "bg-emerald-500/10 text-emerald-700")}>
            {isFailed ? "FAIL" : "OK"}
          </span>
          <span className={cn("flex items-center text-xs text-muted-foreground transition-transform", expanded && "rotate-180")}>
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-zinc-100 shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] text-zinc-400">
            {nodes.length} {nodes.length === 1 ? "node" : "nodes"} · {duration || "—"} · {dayjs(execution.start_at).format("DD MMM HH:mm")}
          </span>
        </div>
      </div>
        ...
        <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-2">
          {nodes.length === 0 && (
            <div className="flex items-center justify-center flex-1 py-8">
              <span className="text-sm text-zinc-400">No logs available for this run</span>
            </div>
          )}
          {nodes.map((node) => (
...
const ActivityTab = ({ assetDetails = {} }) => {
  ...
  const filteredExecutions = useMemo(() => {
    return executions.filter((e) => {
      ...
      if (filter === "succeeded")
        return e.execution_state === "SUCCESS" || e.status === "success" || e.status === "completed";
      if (filter === "failed")
        return e.execution_state === "FAILED" || e.status === "failed" || e.status === "error";
      return true;
    });
  }, [executions, filter]);
  ...
  return (
    <motion.div
      ...
      className="py-1"
      data-testid="workflow-activity-tab"
    >
      <div
      ...
      >
        {FILTER_OPTIONS.map((opt) => (
          <button
            ...
            data-testid={`filter-pill-${opt.key}`}
          >
            {opt.label}
            <span
              className={cn(
        ...
        <button
          ...
          data-testid="refresh-executions-btn"
        >
      ...
      {isLoading && (
        <div className="rounded-xl border border-zinc-100 overflow-hidden" data-testid="activity-loading-skeleton">
          <SkeletonCard />
          <SkeletonCard />
          <SkeletonCard />
        </div>
      ...
      {!isLoading && filteredExecutions.length === 0 && (
        ...
        >
          ...
          <p className="text-xs text-zinc-400 text-center max-w-[280px]">
            Runs will show up here once your workflow executes.
          </p>
        </div>
      )}
      ...
      {!isLoading && filteredExecutions.length > 0 && (
        <div
          ...
          style={{ height: "380px" }}
          data-testid="execution-split-panel"
        >
