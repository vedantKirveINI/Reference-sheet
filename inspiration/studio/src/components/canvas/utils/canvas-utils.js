...
export const updateIfElseNodeLinks = (ifElseRowData) => {
  const ifLinks = (ifElseRowData.ifData || []).map((rowData) => ({
    label: rowData?.conditionStr || "",
    to: rowData?.jumpTo?.key,
    key: rowData?.key,
  }));

  const elseLinks = (ifElseRowData.elseData || []).map((rowData) => ({
    label: rowData?.conditionStr || "ELSE",
    to: rowData?.jumpTo?.key,
    key: rowData?.key,
    linkForElse: true,
  }));

  return [...ifLinks, ...elseLinks];
};
