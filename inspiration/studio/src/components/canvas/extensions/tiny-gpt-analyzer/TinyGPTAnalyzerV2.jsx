...
import { BarChart3, SlidersHorizontal, Settings, Play } from "lucide-react";
...
import { TINYGPT_ANALYZER_V2_NODE, TABS, THEME } from "./constants";
...
const TinyGPTAnalyzerV2 = forwardRef(
  (
    {
      canvasRef,
      annotation,
    ...
    const state = useAnalyzerState(data);
    ...
    const getPrimaryActionLabel = () => {
      ...
      if (activeTab === TABS.CONFIGURE) return "Test Analysis";
      return "Save & Close";
    };
    ...
    const getFooterGuidance = () => {
      if (activeTab === TABS.INITIALISE) {
        if (!state.selectedTemplateId && !state.isFromScratch) {
          return "Select an analysis type or start from scratch";
        }
        return "Click Continue to configure your analysis";
      }
      if (activeTab === TABS.CONFIGURE) {
        if (!state.validation.isValid) {
          ...
          return "Define what content to analyze and what insights to extract";
        }
        return "Define what content to analyze and what insights to extract";
      }
      if (activeTab === TABS.TEST) {
        return "Run a test to validate the analysis output";
      }
      return null;
    };
        ...
        case TABS.TEST:
          return (
            <CommonTestModuleV3
              ...
              parentId={parentId}
              variables={variables}
              node={nodeData || TINYGPT_ANALYZER_V2_NODE}
...
TinyGPTAnalyzerV2.displayName = "TinyGPTAnalyzerV2";

export default TinyGPTAnalyzerV2;
