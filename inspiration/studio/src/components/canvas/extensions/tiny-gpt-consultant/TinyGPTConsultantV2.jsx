...
import { Briefcase, SlidersHorizontal, Settings, Play } from "lucide-react";
...
import { TINYGPT_CONSULTANT_V2_NODE, TABS, THEME } from "./constants";
...
const TinyGPTConsultantV2 = forwardRef(
  (
    {
      canvasRef,
      annotation,
    ...
    const state = useConsultantState(data);
    ...
    const getFooterGuidance = () => {
      if (activeTab === TABS.INITIALISE) {
        if (!state.selectedTemplateId && !state.isFromScratch) {
          return "Select a consultation type or start from scratch";
        }
        return "Click Continue to configure your consultation";
      }
      if (activeTab === TABS.CONFIGURE) {
        if (!state.validation.isValid) {
          const errors = state.validation.errors;
          if (errors.context) return errors.context;
          if (errors.output) return errors.output;
          return "Fill in all required fields to continue";
        }
        return "Define the problem context and desired recommendations";
      }
      if (activeTab === TABS.TEST) {
        return "Run a test to validate the consultation output";
      }
      return null;
    };
        ...
        case TABS.TEST:
          return (
            <CommonTestModuleV3
              ...
              parentId={parentId}
              variables={variables}
              node={nodeData || TINYGPT_CONSULTANT_V2_NODE}
    ...
    return (
      <>
        <WizardDrawer
          ...
          title={data.name || "Tiny GPT Consultant"}
          subtitle="Provide expert advice and recommendations"
          tabs={tabs}
          activeTab={activeTab}
...
TinyGPTConsultantV2.displayName = "TinyGPTConsultantV2";

export default TinyGPTConsultantV2;
