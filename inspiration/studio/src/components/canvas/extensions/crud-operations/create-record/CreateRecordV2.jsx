...
import { CREATE_RECORD_V2_NODE, MYSQL_CREATE_RECORD_NODE, POSTGRES_CREATE_RECORD_NODE, TABS, THEME } from "./constants";
import { useCreateRecordV2State } from "./hooks/useCreateRecordV2State";
...
const CreateRecordV2 = forwardRef(
  (
    {
      canvasRef,
      annotation,
    ...
    const selectedNode = useMemo(() => {
      if (effectiveDatabaseType === DATABASE_TYPES.MYSQL) {
        return MYSQL_CREATE_RECORD_NODE;
      }
      if (effectiveDatabaseType === DATABASE_TYPES.POSTGRESQL) {
        return POSTGRES_CREATE_RECORD_NODE;
      }
      return CREATE_RECORD_V2_NODE; // fallback
    }, [effectiveDatabaseType]);

    const testModuleRef = useRef();
    const drawerRef = useRef();
    const state = useCreateRecordV2State(data, workspaceId, projectId);
    ...
    const getFooterGuidance = () => {
      ...
      if (activeTab === TABS.CONFIGURE && !state.validation.isValid) {
        return "Map at least one field to continue";
      }
        ...
        case TABS.TEST:
          return (
            <CommonTestModuleV3
              ...
              parentId={parentId}
              variables={variables}
              node={nodeData || CREATE_RECORD_V2_NODE}
    ...
    return (
      <WizardDrawer
        ...
        subtitle={CREATE_RECORD_V2_NODE.description}
        tabs={tabs}
        activeTab={activeTab}
...
CreateRecordV2.displayName = "CreateRecordV2";

export default CreateRecordV2;
