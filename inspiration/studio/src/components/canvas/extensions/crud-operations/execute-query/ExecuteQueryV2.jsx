...
import { EXECUTE_V2_NODE, MYSQL_EXECUTE_QUERY_NODE, POSTGRES_EXECUTE_QUERY_NODE, TABS, THEME } from "./constants";
import { useExecuteV2State } from "./hooks/useExecuteV2State";
...
const ExecuteQueryV2 = forwardRef(
  (
    {
      canvasRef,
      annotation,
    ...
    const state = useExecuteV2State(data, workspaceId, projectId);
    ...
    const selectedNode = useMemo(() => {
      if (effectiveDatabaseType === DATABASE_TYPES.MYSQL) {
        return MYSQL_EXECUTE_QUERY_NODE;
      }
      if (effectiveDatabaseType === DATABASE_TYPES.POSTGRESQL) {
        return POSTGRES_EXECUTE_QUERY_NODE;
      }
      return EXECUTE_V2_NODE; // fallback
    ...
    const getFooterGuidance = () => {
      if (activeTab === TABS.INITIALISE && !state.hasInitialised) {
        return "Select a database connection to continue";
      }
      if (activeTab === TABS.CONFIGURE && !state.validation.isValid) {
        return "Enter a SQL query to continue";
      }
    ...
    return (
      <WizardDrawer
        ...
        subtitle={EXECUTE_V2_NODE.description}
        tabs={tabs}
        activeTab={activeTab}
...
ExecuteQueryV2.displayName = "ExecuteQueryV2";

export default ExecuteQueryV2;
