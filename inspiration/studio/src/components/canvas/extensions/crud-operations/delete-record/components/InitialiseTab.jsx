...
import { Database, Table, Check, Zap, Link, Trash2, Loader2 } from "lucide-react";
...
const InitialiseTab = ({
  ...
  const themeColor = "#EF4444"; // Red for delete operations (intentional override)
  ...
  return (
    <div className="space-y-8">
      <div className="text-center space-y-4">
        ...
        >
          <Trash2 className="w-8 h-8" style={{ color: themeColor }} />
        </div>
        <div>
          <h2 className="text-2xl font-semibold text-gray-900">Delete Record</h2>
          <p className="text-gray-500 mt-2 max-w-md mx-auto">
            Remove rows from your {dbConfig.name} database
          </p>
        </div>
      </div>
        ...
        >
          <div className="flex items-center justify-center gap-2">
            ...
            <p className="text-sm font-medium" style={{ color: themeColor }}>
              Connection and table selected. Click Continue to configure delete conditions.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="bg-gray-50 rounded-xl p-4 space-y-3">
          ...
          <ul className="space-y-2 text-sm text-gray-600">
            <li className="flex items-start gap-2">
              ...
              <span><strong>Remove outdated entries</strong> — Clean up old or expired records</span>
            </li>
            <li className="flex items-start gap-2">
              <span style={{ color: themeColor }} className="mt-0.5">•</span>
              <span><strong>Process cancellations</strong> — Delete orders, bookings, or subscriptions</span>
            </li>
            <li className="flex items-start gap-2">
              <span style={{ color: themeColor }} className="mt-0.5">•</span>
              <span><strong>Data cleanup</strong> — Remove duplicates or invalid entries automatically</span>
            </li>
          </ul>
        </div>
      )}
