...
import { Database, Table, Check, Zap, Link, Search, Loader2 } from "lucide-react";
  ...
  return (
    <div className="space-y-8">
      <div className="text-center space-y-4">
        ...
        >
          <Search className="w-8 h-8" style={{ color: themeColor }} />
        </div>
        <div>
          <h2 className="text-2xl font-semibold text-gray-900">Find One Record</h2>
          <p className="text-gray-500 mt-2 max-w-md mx-auto">
            Retrieve a single row from your {dbConfig.name} database
          </p>
        </div>
      </div>
      ...
      {connection && table && (
        ...
        >
          <p className="text-sm font-medium" style={{ color: themeColor }}>
            Connection and table selected. Click Continue to configure your search.
          </p>
        </div>
      )}
        ...
        <div className="bg-gray-50 rounded-xl p-4 space-y-3">
          ...
          <ul className="space-y-2 text-sm text-gray-600">
            <li className="flex items-start gap-2">
              ...
              <span><strong>Look up by ID</strong> — Fetch a specific record using its unique identifier</span>
            </li>
            <li className="flex items-start gap-2">
              <span style={{ color: themeColor }} className="mt-0.5">•</span>
              <span><strong>Get user profile</strong> — Retrieve details for a single user or account</span>
            </li>
            <li className="flex items-start gap-2">
              <span style={{ color: themeColor }} className="mt-0.5">•</span>
              <span><strong>Check existence</strong> — Verify if a record exists before taking action</span>
            </li>
          </ul>
        </div>
      )}
