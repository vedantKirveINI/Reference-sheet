...
import { GPT_CONSULTANT_NODE, TABS, THEME } from "./constants";
import { useGPTConsultantState } from "./hooks/useGPTConsultantState";
...
const GPTConsultant = forwardRef(
  (
    {
      canvasRef,
      annotation,
    ...
    const state = useGPTConsultantState(data);
    ...
    const getFooterGuidance = () => {
      if (activeTab === TABS.INITIALISE) {
        if (!state.selectedTemplateId) {
          return "Select a template or start from scratch to define your consulting task";
        }
      ...
      if (activeTab === TABS.CONFIGURE) {
        ...
        return "Configure your consulting prompt and output format, then test it";
      }
        ...
        case TABS.TEST:
          return (
            <CommonTestModuleV3
              ...
              parentId={parentId}
              variables={variables}
              node={nodeData || GPT_CONSULTANT_NODE}
    ...
    return (
      <>
        <WizardDrawer
          ...
          title={data.name || "GPT Consultant"}
          subtitle="Get expert advice and recommendations"
          tabs={tabs}
          activeTab={activeTab}
...
GPTConsultant.displayName = "GPTConsultant";

export default GPTConsultant;
