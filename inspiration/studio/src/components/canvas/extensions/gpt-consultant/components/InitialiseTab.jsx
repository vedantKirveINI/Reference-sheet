...
import { Briefcase, Code, Users, Settings } from "lucide-react";
    ...
    <div className="space-y-8 p-6">
      <div className="text-center space-y-4">
        <div className="mx-auto w-16 h-16 bg-gradient-to-br from-orange-500 to-orange-600 rounded-2xl flex items-center justify-center shadow-lg">
          <img src={extensionIcons.tinyGptConsultant} alt="GPT Consultant" className="w-10 h-10" />
        </div>
        <div>
          <h2 className="text-2xl font-semibold text-gray-900">GPT Consultant</h2>
      ...
      <div className="bg-orange-50 rounded-xl p-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          <UseCaseItem icon={Briefcase} text="Business strategy" />
          <UseCaseItem icon={Code} text="Technical reviews" />
          <UseCaseItem icon={Users} text="HR advice" />
          <UseCaseItem icon={Settings} text="Process optimization" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {CONSULTANT_TEMPLATES.map((template) => (
          <GPTTemplateCard
            key={template.id}
            template={template}
