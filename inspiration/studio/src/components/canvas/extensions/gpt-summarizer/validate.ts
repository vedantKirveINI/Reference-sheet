...
export const validateGPTSummarizer = (data: GPTSummarizerData): ValidationResult => {
  ...
  if (!hasPrompt) {
    errors.prompt = "Provide a summarization prompt";
  }
...
export const normalizeOutputSchema = (schema: OutputSchemaField[]): OutputSchemaField[] => {
  if (!Array.isArray(schema)) {
    return [{ id: "field-default", key: "summary", type: "string", required: true }];
  }
...
export default validateGPTSummarizer;
