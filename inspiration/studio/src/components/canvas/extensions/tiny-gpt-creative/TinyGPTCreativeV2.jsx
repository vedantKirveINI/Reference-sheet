...
import { TINYGPT_CREATIVE_V2_NODE, TABS, THEME } from "./constants";
...
const TinyGPTCreativeV2 = forwardRef(
  (
    {
      canvasRef,
      annotation,
    ...
    const state = useCreativeState(data);
    ...
    const getFooterGuidance = () => {
      if (activeTab === TABS.INITIALISE) {
        if (!state.selectedTemplateId && !state.isFromScratch) {
          return "Choose a creative style or start from scratch";
        }
        return "Click Continue to configure your creative task";
      }
      if (activeTab === TABS.CONFIGURE) {
        if (!state.validation.isValid) {
          ...
          return "Define your creative task and desired output format";
        }
        return "Configure your creative task, then test it";
      }
      if (activeTab === TABS.TEST) {
        return "Run a test to preview the creative output";
      }
      return null;
    };
        ...
        case TABS.TEST:
          return (
            <CommonTestModuleV3
              ...
              parentId={parentId}
              variables={variables}
              node={nodeData || TINYGPT_CREATIVE_V2_NODE}
    ...
    return (
      <>
        <WizardDrawer
          ...
          title={data.name || "GPT Creative"}
          subtitle="Generate creative content like stories, poetry, and marketing copy"
          tabs={tabs}
          activeTab={activeTab}
...
TinyGPTCreativeV2.displayName = "TinyGPTCreativeV2";

export default TinyGPTCreativeV2;
