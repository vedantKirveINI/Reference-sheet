...
import { GPT_TRANSLATOR_NODE, TRANSLATOR_TEMPLATES, TABS, THEME } from "./constants";
...
const GPTTranslator = forwardRef(
  (
    {
      canvasRef,
      annotation,
    ...
    const state = useGPTState(data, TRANSLATOR_TEMPLATES);
    ...
    const getFooterGuidance = () => {
      if (activeTab === TABS.INITIALISE) {
        if (!state.selectedTemplateId) {
          return "Select a template or start from scratch to configure your translator";
        }
      ...
      if (activeTab === TABS.CONFIGURE) {
        ...
        return "Configure your translation settings, then test it";
      }
      if (activeTab === TABS.TEST) {
        return "Run a test to validate the translation before saving";
      }
      return null;
    };
        ...
        case TABS.TEST:
          return (
            <CommonTestModuleV3
              ...
              parentId={parentId}
              variables={variables}
              node={nodeData || GPT_TRANSLATOR_NODE}
    ...
    return (
      <>
        <WizardDrawer
          ...
          subtitle="Translate content between languages"
          tabs={tabs}
          activeTab={activeTab}
...
GPTTranslator.displayName = "GPTTranslator";

export default GPTTranslator;
