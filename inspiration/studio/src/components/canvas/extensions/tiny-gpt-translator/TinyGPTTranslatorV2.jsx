...
import { Languages, SlidersHorizontal, Settings, Play } from "lucide-react";
...
import { TINYGPT_TRANSLATOR_V2_NODE, TABS, THEME } from "./constants";
...
const TinyGPTTranslatorV2 = forwardRef(
  (
    {
      canvasRef,
      annotation,
    ...
    const state = useTranslatorState(data);
    ...
    const getFooterGuidance = () => {
      if (activeTab === TABS.INITIALISE) {
        if (!state.selectedTemplateId && !state.isFromScratch) {
          return "Select language pair or start from scratch";
        }
        return "Click Continue to configure your translation";
      }
      if (activeTab === TABS.CONFIGURE) {
        if (!state.validation.isValid) {
          const errors = state.validation.errors;
          if (errors.inputText) return errors.inputText;
          if (errors.targetLanguage) return errors.targetLanguage;
          if (errors.output) return errors.output;
          return "Specify source text and translation options";
        }
        return "Configure your translation options, then test it";
      }
      if (activeTab === TABS.TEST) {
        return "Run a test to validate the translation quality";
      }
      return null;
    };
        ...
        case TABS.TEST:
          return (
            <CommonTestModuleV3
              ...
              parentId={parentId}
              variables={variables}
              node={nodeData || TINYGPT_TRANSLATOR_V2_NODE}
    ...
    return (
      <>
        <WizardDrawer
          ...
          title={data.name || "GPT Translator"}
          subtitle="Translate text between languages while preserving tone"
          tabs={tabs}
          activeTab={activeTab}
...
TinyGPTTranslatorV2.displayName = "TinyGPTTranslatorV2";

export default TinyGPTTranslatorV2;
