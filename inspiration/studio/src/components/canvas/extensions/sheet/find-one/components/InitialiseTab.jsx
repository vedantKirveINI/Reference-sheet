...
import tinyTablesFindOneIcon from "@/components/canvas/assets/extensions/tiny-tables-find-one.svg";
...
const InitialiseTab = ({
  ...
  const getDynamicTemplates = useMemo(() => {
    const templates = [
      {
        id: "find-by-id",
        name: "Find by ID",
        description: "Look up a record by its unique identifier",
        icon: "Hash",
      },
    ];
    ...
    const emailField = sortedFields.find(f =>
      ['email', 'mail', 'e-mail'].some(kw =>
    ...
    if (emailField) {
      templates.push({
        id: "lookup-by-email",
        name: `Lookup by ${emailField.name}`,
        description: `Find a specific record by ${emailField.name.toLowerCase()}`,
        icon: "Mail",
    ...
    if (dateField) {
      templates.push({
        id: "find-most-recent",
        name: `Find Most Recent by ${dateField.name}`,
        description: `Locate the latest record by ${dateField.name.toLowerCase()}`,
        icon: "Clock",
    ...
    <div className="space-y-8">
      <div className="text-center space-y-4">
        <div className="mx-auto w-16 h-16 bg-[#22C55E]/10 rounded-2xl flex items-center justify-center">
          <img src={tinyTablesFindOneIcon} alt="Find One Record" className="w-8 h-8" />
        </div>
        <div>
          <h2 className="text-2xl font-semibold text-gray-900">Find One Record</h2>
          <p className="text-gray-500 mt-2 max-w-md mx-auto">
            Find a single row that matches your criteria
          </p>
        </div>
      </div>
        ...
        <>
          <div className="bg-gray-50 rounded-xl p-4 space-y-3">
            <h3 className="font-medium text-gray-900 flex items-center gap-2">
              ...
              When to use Find One Record
            </h3>
            <ul className="space-y-2 text-sm text-gray-600">
              <li className="flex items-start gap-2">
                <span className="text-[#22C55E] mt-0.5">•</span>
                <span><strong>Lookup customer</strong> — Find a specific customer by ID or email</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-[#22C55E] mt-0.5">•</span>
                <span><strong>Get order details</strong> — Retrieve a single order record</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-[#22C55E] mt-0.5">•</span>
                <span><strong>Find specific record</strong> — Locate a unique row by any field</span>
              </li>
            </ul>
          </div>
          ...
          >
            ...
            <p className="text-sm text-gray-500 mt-1">Configure your own lookup settings</p>
          </button>
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <div className={cn(
                        ...
                        <img src={tinyTablesFindOneIcon} alt={template.name} className="w-5 h-5" />
                      </div>
