...
import tinyTablesCreateIcon from "@/components/canvas/assets/extensions/tiny-tables-create.svg";
...
const InitialiseTab = ({
  ...
  const getDynamicTemplates = useMemo(() => {
    const templates = [
      {
        id: "create-form-submission",
        name: "Create Form Submission",
        description: "Store data from form responses automatically",
        icon: "UserPlus",
      },
    ];
    ...
    const dateField = sortedFields.find(f =>
      ['date', 'created', 'updated', 'time', 'timestamp'].some(kw =>
    ...
    if (dateField) {
      templates.push({
        id: "log-event",
        name: `Log Event with ${dateField.name}`,
        description: `Track activities with ${dateField.name.toLowerCase()} timestamps`,
        icon: "Calendar",
        fieldId: dateField.id,
      });
    }

    const textField = sortedFields.find(f =>
      ['name', 'title', 'description', 'note'].some(kw =>
        f.name.toLowerCase().includes(kw)
    ...
    if (textField) {
      templates.push({
        id: "store-data",
        name: `Store ${textField.name} Data`,
        description: `Save workflow outputs to ${textField.name.toLowerCase()} field`,
        icon: "Database",
    ...
    <div className="space-y-8">
      <div className="text-center space-y-4">
        <div className="mx-auto w-16 h-16 bg-[#22C55E]/10 rounded-2xl flex items-center justify-center">
          <img src={tinyTablesCreateIcon} alt="Create Record" className="w-8 h-8" />
        </div>
        <div>
          <h2 className="text-2xl font-semibold text-gray-900">Create Record</h2>
        ...
        <>
          <div className="bg-gray-50 rounded-xl p-4 space-y-3">
            <h3 className="font-medium text-gray-900 flex items-center gap-2">
              ...
              When to use Create Record
            </h3>
            <ul className="space-y-2 text-sm text-gray-600">
              <li className="flex items-start gap-2">
                <span className="text-[#22C55E] mt-0.5">•</span>
                <span><strong>Save form submissions</strong> — Store data from form responses automatically</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-[#22C55E] mt-0.5">•</span>
                <span><strong>Log events</strong> — Track activities and create audit trails</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-[#22C55E] mt-0.5">•</span>
                <span><strong>Store data</strong> — Save workflow outputs for later use</span>
              </li>
            </ul>
          </div>
          ...
          >
            ...
            <p className="text-sm text-gray-500 mt-1">Configure your own record settings</p>
          </button>
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <div className={cn(
                        ...
                        <img src={tinyTablesCreateIcon} alt={template.name} className="w-5 h-5" />
                      </div>
