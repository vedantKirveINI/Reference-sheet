...
const getOAuth2UrlFromConfig = async (configs: any[] = []) => {
  ...
  const authUrl = `${oAuthEndPointConfig.value}?client_id=${
    client_id.value
  }&redirect_uri=${redirect_uri.value}&response_type=${response_type.value}${
...
const StripeConnectionItem = ({
  ...
  return (
    <div
      className={cn(
        "grid grid-cols-[auto_1fr_auto] items-center gap-4 p-3 rounded-lg",
        isSelected
          ? "border-2 border-[#212121] bg-[#f5f5f5]"
