...
import { GPT_CREATIVE_NODE, CREATIVE_TEMPLATES, TABS, THEME } from "./constants";
...
const GPTCreative = forwardRef(
  (
    ...
    const state = useGPTState(data, CREATIVE_TEMPLATES);
    ...
    const getFooterGuidance = () => {
      if (activeTab === TABS.INITIALISE) {
        if (!state.selectedTemplateId) {
          return "Select a template or start from scratch to create your content";
        }
      ...
      if (activeTab === TABS.CONFIGURE) {
        ...
        return "Configure your creative content settings, then test it";
      }
      if (activeTab === TABS.TEST) {
        return "Run a test to validate the generated content before saving";
      }
      return null;
    };
        ...
        case TABS.TEST:
          return (
            <CommonTestModuleV3
              ...
              parentId={parentId}
              variables={variables}
              node={nodeData || GPT_CREATIVE_NODE}
    ...
    return (
      <>
        <WizardDrawer
          ...
          subtitle="Generate creative content and copy"
          tabs={tabs}
          activeTab={activeTab}
...
GPTCreative.displayName = "GPTCreative";

export default GPTCreative;
