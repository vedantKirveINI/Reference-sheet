...
const CommonTestModule = forwardRef(
  (
    ...
    }, [checkForInputs, inputMode, runTest, onTestStart, go_data]);
        ...
        switch (data?.type) {
          ...
          case SOCKET_EVENTS.SOCKET_EXECUTE_NODE_ERROR_KEY: {
            ...
            toast.error("Test Execution Error", {
              description: data?.result?.message || "An error occurred during test execution",
            });
            break;
          }
          default:
    ...
    useImperativeHandle(
      ...
      [debouncedBeginTest, isProcessing, inputMode]
    );
    ...
    useEffect(() => {
      const initSocket = async () => {
        ...
        try {
          ...
          setSocket(result);
          console.log("[SOCKET_CREATED] CommonTestModule (legacy node Run, execute_node)", {
            source: "CommonTestModule.jsx",
            socket_id: result?.id,
            node_key: node?.key,
          });
        } catch (error) {
