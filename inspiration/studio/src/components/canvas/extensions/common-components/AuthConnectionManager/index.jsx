...
const AuthConnectionManager = ({
  ...
  const onConnectionDeleteHandler = useCallback(
    async (deleteConnectionId) => {
      const response =
        await authorizeDataSDKServices.deleteById(deleteConnectionId);
  ...
  const rootClassName = variant === "inline" 
    ? `${classes["connection-manager-root"]} ${classes["connection-manager-root-inline"]}`
    : classes["connection-manager-root"];

  const containerClassName = variant === "inline"
    ? `${classes["connection-manager"]} ${classes["connection-manager-inline"]}`
    : classes["connection-manager"];

  return (
    <section
      className={rootClassName}
      data-testid="connection-manager-root"
    >
      {!hideTitle && (
        <h3
          className={classes["connection-manager-title"]}
      ...
      <div className={containerClassName}>
        <Renderer
          ...
          integrationIcon={nodeData?.meta?.thumbnail || nodeData?.icon || nodeData?.logo}
        />
      </div>
    </section>
  );
