...
import { GraduationCap, BookOpen, Lightbulb, FileText, CheckCircle } from "lucide-react";
import { LEARNING_TEMPLATE_PRESETS } from "../templates/presets";
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "border-2",
        isSelected
          ? "border-green-500 shadow-[0_2px_12px_rgba(34,197,94,0.25)]"
    ...
    <div className="space-y-8 p-6">
      <div className="text-center space-y-4">
        <div className="mx-auto w-16 h-16 bg-gradient-to-br from-green-500 to-green-600 rounded-2xl flex items-center justify-center shadow-lg">
          <img src={extensionIcons.tinyGptLearning} alt="Tiny GPT Learning" className="w-10 h-10" />
        </div>
        <div>
          <h2 className="text-2xl font-semibold text-gray-900">Tiny GPT Learning</h2>
      ...
      <div className="bg-green-50 rounded-xl p-4">
        <h3 className="text-sm font-medium text-green-900 mb-3">When to use</h3>
        <div className="grid grid-cols-2 gap-2">
          <UseCaseItem icon={BookOpen} text="Explain concepts" />
          <UseCaseItem icon={Lightbulb} text="Create tutorials" />
          <UseCaseItem icon={FileText} text="Write lessons" />
          <UseCaseItem icon={CheckCircle} text="Build quizzes" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {LEARNING_TEMPLATE_PRESETS.map((template) => (
          <TemplateCard
            key={template.id}
            template={template}
